-- Fix green coffee inventory calculation to properly aggregate by coffee name
-- The issue was that grouping by entity_id AND metadata caused separate groups
-- for the same coffee with different metadata (different purchase dates, suppliers, etc.)
